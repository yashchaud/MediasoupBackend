...
module.exports = async function (io) {
  ...
  const createRoom = async (roomName, socketId) => {
    return roomQueue.push(async () => {
      ...
      let peers = [];
      if (!room) {
        const router = await workermap.get(0).router;
    ...
    producers.forEach((producerData) => {
      if (
        ...
        if (peers.has(producerData.socketId)) {
          ...
          if (alreadyPipedProducer.size > 0) {
            for (const pipeProducer of alreadyPipedProducer) {
              producerSocket.emit("new-producer", { producerId: pipeProducer });
            }
          }
        } else {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("getProducers", async (callback) => {
      ...
      // Move the pipeProducersBetweenRouters call outside of the forEach/map
      console.log("workermap contents:", workermap);
      try {
        const basemap = workermap.get(0);
        if (!basemap) {
          throw new Error(
            "Base map entry for key 0 is not found in workermap."
          );
        }
        const sourceRouter = basemap.router;
        const targetbase = workermap.get(1);
        if (!targetbase) {
          throw new Error(
            "Target base entry for key 1 is not found in workermap."
          );
        }
        const targetRouter = targetbase.router;
        console.log(`This is source Router 21`, sourceRouter);

        await pipeProducersBetweenRouters({
          producerIds: producerList,
          sourceRouter,
          targetRouter,
